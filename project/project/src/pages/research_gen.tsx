...
const ResearchPage = () => {
  const ngrok_link = "https://7142-104-196-244-73.ngrok-free.app/";
  ...
  const [researchTitle, setResearchTitle] = React.useState("FedxData");
  ...
  const handlePodcastProcess = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        // Set the Cloudinary URL from the backend response
  ...
  const handleComicProcess = async () => {
    ...
    try {
      const response = await fetch("https://10bd-34-105-43-138.ngrok-free.app/predict", {
        method: "POST",
        body: formData,
      });
      ...
      if (result.error) {
        ...
        // Updated to use the correct property "comic_url"
        setComicUrl(result.comic_url);
      }
    } catch (error) {
  ...
  return (
    <div className="mt-4 max-w-4xl mx-auto px-4 py-8">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 ">
            {outputTypes.map(({ id, name, icon: Icon }) => (
              <button
                ...
                className={`bg-white p-4 rounded-lg border-2 transition-colors flex flex-col items-center justify-center ${
                  selectedOutput === id
                    ? "border-[#382D76] bg-[#382D76]/10"
